...
$queryset = mysqli_query($connection, "INSERT into marks VALUES('$semister','$dept','$section','$suid','','$uid','','','','','','','','','','')");
...
$uid = $_SESSION['user_id'];
if($suid = "ALL"){
$queryset = mysqli_query($connection, "select * from marks where sid = '$uid'");
	}
	else{
$queryset = mysqli_query($connection, "select * from marks where suid = '$suid' and sid = '$uid'");
	}
confirm_query($queryset);
$mark = mysqli_fetch_array($queryset);
}
if(isset($mark)){
	...
	while($mark = mysqli_fetch_array($queryset)){
